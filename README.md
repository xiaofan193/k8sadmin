#Sponge项目开发脚手架 + client-go + vue3 Element-plus 开发一个简单的Kubernetes图形化管理平台

阶段一: kubernetes 资源管理
阶段二： 支持线上配置yaml，声明式
阶段三： 前端数据渲染联调（后期有时间再做）
开发环境说明：
...
- 开发工具：goland
- go框架： Sponge
- web框架: vue3 + Element-plus
- kubernetes: kind搭建Kubernetes集群
## 项目的初始化
...
```bash
go get k8s.io/client-go@v0.33.2
```
