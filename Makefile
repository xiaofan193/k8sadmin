...
graph:
	...
	@cp -f cmd/userapi/main.go .
	go-callvis -skipbrowser -format=svg -nostd -file=userapi users
	@rm -f main.go userapi.gv
...
.PHONY: build
# Build userapi for linux amd64 binary
build:
	@echo "building 'userapi', linux binary file will output to 'cmd/userapi'"
	@cd cmd/userapi && CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build
