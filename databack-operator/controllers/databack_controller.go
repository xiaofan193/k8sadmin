...
import (
	...
	operatorkubeimooccomv1beta1 "kubeimooc.com/operator-databackup/api/v1beta1"
	"os"
	"os/exec"
	"reflect"
...
type DatabackReconciler struct {
	...
	BackupQueue map[string]operatorkubeimooccomv1beta1.Databack
	Wg          sync.WaitGroup
	Tickers     []*time.Ticker
	lock        sync.RWMutex
}

//+kubebuilder:rbac:groups=operator.kubeimooc.com,resources=databacks,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=operator.kubeimooc.com,resources=databacks/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=operator.kubeimooc.com,resources=databacks/finalizers,verbs=update
...
func (r *DatabackReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err != nil {
		if errors.IsNotFound(err) {
			...
			//thanks@https://coding.imooc.com/learn/questiondetail/298952.html
...
func (r *DatabackReconciler) DeleteQueue(name string) {
	delete(r.BackupQueue, name)
	r.StopLoop()
	go r.RunLoop()
}
func (r *DatabackReconciler) AddQueue(databack operatorkubeimooccomv1beta1.Databack) {
	if r.BackupQueue == nil {
		r.BackupQueue = make(map[string]operatorkubeimooccomv1beta1.Databack)
	}
...
func (r *DatabackReconciler) RunLoop() {
	for name, databack := range r.BackupQueue {
		...
		delay := r.getDelaySeconds(databack.Spec.StarTime)
		if delay.Hours() < 1 {
		...
		r.Wg.Add(1)
		go func(databack operatorkubeimooccomv1beta1.Databack) {
			defer r.Wg.Done()
			for {
				<-ticker.C
				//重置ticker
...
func (r *DatabackReconciler) UpdateStatus(backup operatorkubeimooccomv1beta1.Databack) {
	r.lock.Lock()
	defer r.lock.Unlock()
	ctx := context.TODO()
	...
	var dataBackupK8s operatorkubeimooccomv1beta1.Databack
...
// mysql数据导出+数据上报到OSS
func (r *DatabackReconciler) DumpWithUploadOss(backup operatorkubeimooccomv1beta1.Databack) error {
	defer func() {
...
func (r *DatabackReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&operatorkubeimooccomv1beta1.Databack{}).
		Complete(r)
}
