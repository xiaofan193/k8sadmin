...
func mapToList(m map[string]string) []types.ListMapItem {
	res := make([]types.ListMapItem, 0)
	for k, v := range m {
		res = append(res, types.ListMapItem{
			Key:   k,
			Value: v,
		})
	}
...
func (this *NodeK8s2Res) GetNodeDetail(nodeK8s *corev1.Node) *types.Node {
...
func (*NodeK8s2Res) GetNodeResItem(nodeK8s *corev1.Node) *types.Node {
	...
	return &types.Node{
		...
		Labels:           mapToList(nodeK8s.Labels),
	}
}
